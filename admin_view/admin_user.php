<?php
    ...
    if(isset($_GET['delete_all'])){
        mysqli_query($conn, "DELETE FROM `usuarios`");
        header('location:admin_user.php');
    }

    if(isset($_GET['remove'])){
        $remove_id = $_GET['remove'];
        mysqli_query($conn, "DELETE FROM `usuarios` WHERE id = '$remove_id'");
        header('location:admin_user.php');
     };
...
<body>
    ...
    <main>
        <div class="banner">
            <span>˗ˏˋ Usuarios ⁺𓂃</span>
        </div>

        <div class="box-container">
            <?php
                $select_user=mysqli_query($conn, "SELECT * FROM `usuarios` WHERE tipo = 'usuario'") or die('query failed');
                if(mysqli_num_rows($select_user)>0){
                    while($fetch_user = mysqli_fetch_assoc($select_user)){

            ?>
            <div class="box">
                <p> ID: <br> <span><?php echo $fetch_user['id'];?> </span> </p> <br>
                <p> NOMBRE: <br> <span><?php echo $fetch_user['nombre'];?> </span> </p> <br>
                <p> EMAIL: <br> <span><?php echo $fetch_user['email'];?> </span> </p> <br>
                <p> CONTRASEÑA: <br> <span><?php echo $fetch_user['contrase'];?><span></p> <br> <br>
        
                <td><a href="admin_user.php?remove=<?php echo $fetch_user['id']; ?>" onclick="return confirm('¿Estás seguro de borrar este usuario? Σ (O_O;)');" class="delete-btn"> <i class="fas fa-trash"></i>Borrar Usuario</a></td>
